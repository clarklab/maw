...
export class Lane {
  ...
  render() {
    ...
    for (const it of this.items) {
      ...
      if (it.type === 'word') {
        // comet tail
        const tail = ctx.createLinearGradient(0, y, 0, y + 30);
        tail.addColorStop(0, 'rgba(255, 214, 120, 0.55)');
        tail.addColorStop(1, 'rgba(255, 214, 120, 0)');
        ctx.strokeStyle = tail;
        ctx.lineWidth = 3;
        ctx.beginPath();
        ctx.moveTo(TRACK_X, y + 4);
        ctx.lineTo(TRACK_X, y + 30);
        ctx.stroke();
        ctx.shadowColor = 'rgba(255, 214, 120, 0.9)';
        ctx.shadowBlur = 12;
        ctx.fillStyle = '#ffd878';
        ctx.beginPath(); ctx.arc(TRACK_X, y, 6.5, 0, 7); ctx.fill();
        ctx.shadowBlur = 0;
        ctx.fillStyle = '#fff8ea';
        ctx.beginPath(); ctx.arc(TRACK_X, y, 2.6, 0, 7); ctx.fill();
      } else {
        ...
        if (it.warn) {
          ...
          ctx.lineWidth = 2;
          ctx.beginPath(); ctx.arc(TRACK_X + wob, y, 8 + ringT * 9, 0, 7); ctx.stroke();
        }
        ctx.shadowColor = 'rgba(255, 90, 50, 0.7)';
        ctx.shadowBlur = it.warn ? 10 : 6;
        ctx.fillStyle = it.color || '#c84a30';
        ctx.beginPath(); ctx.arc(TRACK_X + wob, y, 6, 0, 7); ctx.fill();
        ctx.shadowBlur = 0;
        ctx.strokeStyle = 'rgba(255, 245, 235, 0.85)';
        ctx.lineWidth = 1.6;
        ctx.beginPath(); ctx.arc(TRACK_X + wob, y, 6, 0, 7); ctx.stroke();
      }
    }
    ctx.globalAlpha = 1;
  }
